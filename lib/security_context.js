...
const _toRegExpList = (value, argName) => {
  if (value === null || value === undefined) {
    console.log('*** DEBUG adding ' + argName + ' == null value');
    return [];
  }
  if (util.isString(value)) {
    console.log('*** DEBUG adding ' + argName + ' == ' + value);
    return [_strAsRegExp(value)];
  }
  if (util.isRegExp(value)) {
    console.log('*** DEBUG adding ' + argName + ' == regex ' + value);
    return [value];
  }
  if (util.isArray(value)) {
    const ret = [];
    for (var i = 0; i < value.length; i++) {
      console.log('*** DEBUG adding ' + argName + '[' + i + '] == ' + value[i]);
...
const _isMatched = (value, regexpList) => {
  ...
  for (var i = 0; i < regexpList.length; i++) {
    ...
    console.log('*** DEBUG `' + value + '` did not match `' +
      regexpList[i] + '`');
  }
  ...
  constructor(options) {
    for (var k in options) {
      if (options.hasOwnProperty(k)) {
        console.log('FAC options[' + k + ']=' + (typeof options[k]));
      }
    }
    Object.defineProperties(this, {
      ...
      [kListable]: {
        enumerable: false,
        writable: false,
        value: _toRegExpList(options.listable, 'listable')
      },
      [kContext]: {
        enumerable: false,
        writable: false,
...
const addFileAccessController = (controllerObj, options) => {
  if (options === undefined) {
    console.log('*** DEBUG shifting the options over one');
    options = controllerObj;
    controllerObj = {};
    for (var k in options) {
      if (options.hasOwnProperty(k)) {
        console.log('*** DEBUG options[' + k + ']=' + options[k] +
          ' (' + (typeof options[k]) + ')');
      }
    }
  }
