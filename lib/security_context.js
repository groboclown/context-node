...
const _getResourceArg = (resourceDef, args) => {
  if (typeof resourceDef === 'string') {
    const match = _ARGUMENT_RE.exec(resourceDef);
    if (match) {
  ...
  onContext(invoker) {
    ...
    if (util.isString(this[kContext].flags) && util.isString(path)) {
      ...
      if (flags.indexOf('r') >= 0 || flags.indexOf('+') > 0) {
        _checkMode(path, this[kReadable], 'ERR_FILE_READ_FORBIDDEN');
      }
      if (flags.indexOf('w') >= 0 || flags.indexOf('a') >= 0 ||
          flags.indexOf('+') > 0) {
        _checkMode(path, this[kWritable], 'ERR_FILE_WRITE_FORBIDDEN');
      }
    }
    ...
    for (i = 0; i < this[kContext].list.length; i++) {
      ...
      _checkMode(path, this[kListable], 'ERR_FILE_LIST_FORBIDDEN');
    }
