...
const DEBUG = process.env.NODE_DEBUG && /security/.test(process.env.NODE_DEBUG);
  ...
  if (util.isArray(value)) {
    ...
    for (var i = 0; i < value.length; i++) {
      if (typeof value[i] === 'string') {
        ...
        // FIXME DEBUG
        console.log('argument value = `' + value[i] + '`');
        const errors = lazyErrors();
        throw new errors.TypeError(
          'ERR_INVALID_ARG_TYPE', argName + '[' + i + ']',
...
const _isMatched = (value, matcher) => {
  if (typeof value !== 'string') {
    if (DEBUG) {
      const errors = lazyErrors();
      throw new errors.TypeError('ERR_INVALID_ARG_TYPE', 'value',
                                 'string');
    }
    return false;
  }
